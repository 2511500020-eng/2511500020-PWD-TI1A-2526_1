...
document.querySelectorAll("form").forEach(form =>
    form.addEventListener("submit", function (e) {
        ...
        if (form.id === "formDaftar") {
            const namaLengkap = document.getElementById("txtNamaLengkap");
            e.preventDefault();
            let isValid = true;

            document.querySelectorAll(".error-msg").forEach(el => el.remove());
            [namaLengkap].forEach(el => el.style.border = "");

            if (namaLengkap.value.trim() === "") {
                showError(namaLengkap, "Nama wajib diisi.");
                isValid = false;
            }
